...
def generate_alignment_html(alignment, scores, output_html, palette, metric):
    """
    Generate an ESPript-like HTML alignment viewer with sticky scroll containers 
    and truncated names, configured for infinite horizontal scrolling.
    """
...
/* CSS Truncation and Sticky Properties */
.name{
    font-weight:bold;
    padding-right:10px;
    padding-left:10px;
...
.num{
    ...
    text-align:center;
    color:#555;
    min-width: 16px; /* Ensures numbers don't squish */
}
...
""")
    ...
    # FORCE INFINITE HORIZONTAL SCROLLING 
    # By setting the block size to the total alignment length
    block_size = aln_len 

    for start in range(0, aln_len, block_size):
        end = min(start + block_size, aln_len)
        
        # Wrapped the table in a container
        ...
        for i in range(start, end):
            ...
            if aa != "-" and tmp_number % 10 == 0:
                html.append(f"<td class='num' colspan='1'>{tmp_number}</td>")
            else:
        ...
        for seq_index, record in enumerate(alignment):
            ...
            # Python truncation for the sequence string [:20]
            # Included full name in title for hover ability
            truncated_name = record.id[:20]
            html.append(f"<td class='name' title='{record.id}'>{truncated_name}</td>")
...
def process_structure(alignment_file, structure_file, output_dir, metric, gap_penalty, palette, step_value):
    ...
    # Robustly parse FASTA/A3M, handling variable lengths and duplicate IDs.
    with open(alignment_file, "r") as handle:
        try:
            # First, try to read as a standard, aligned FASTA.
            alignment = AlignIO.read(handle, "fasta")
        except ValueError as e:
            if "Sequences must all be the same length" in str(e):
                # This error indicates an A3M-like file.
                # Rewind the file handle to parse from the beginning.
                handle.seek(0)
                
                # FIX: Use SeqIO instead of AlignIO for variable length sequences
                records = list(SeqIO.parse(handle, "fasta"))
                if not records:
                    raise ValueError("Alignment file is empty or not in FASTA format.")
                
                # Format A3M into a standard strict alignment
                ref_len = len(records[0].seq)
                for record in records:
                    # 1. Remove lowercase letters (A3M insertion states)
                    seq_str = "".join(c for c in str(record.seq) if not c.islower())
                    
                    # 2. Pad with gaps if it's shorter, or truncate if it's somehow longer
                    seq_str = seq_str.ljust(ref_len, '-')[:ref_len]
                    
                    # 3. Reassign as a Biopython Seq object
                ...
                alignment = AlignIO.MultipleSeqAlignment(unique_records)
            else:
    ...
    # Validate sequence before mapping
    validate_sequence(residues, ref_seq)

    # Filter out gaps and lowercase (insertions) from the reference sequence
    # when mapping scores to the structure.
    mapped_scores_for_pdb = []
    for i, aa in enumerate(ref_seq):
        if aa.isupper(): # Only consider uppercase amino acids for mapping to PDB
            mapped_scores_for_pdb.append(scores[i])

    for residue, score in zip(residues, mapped_scores_for_pdb):
    ...
    with open(txt_path, "w") as f:
        ...
        f.write(f"# Gap/Insertion penalty applied: {gap_penalty}\n")
...
def validate_sequence(residues, ref_seq_from_alignment):
    """
    Validates that the sequence extracted from the PDB structure matches the
    first sequence of the MSA, after filtering out gaps and lowercase insertions
    from the MSA sequence.
    """
    structure_seq = "".join(seq1(r.get_resname(), custom_map={}) for r in residues)
    
    # Filter out gaps and lowercase (insertions) from the reference sequence
    msa_seq_filtered = "".join(aa for aa in ref_seq_from_alignment if aa.isupper())

    if structure_seq != msa_seq_filtered:
        error_msg = (
            "Sequence mismatch between structure and alignment.\n"
            f"PDB sequence length: {len(structure_seq)}\n"
            f"MSA (filtered) sequence length: {len(msa_seq_filtered)}\n"
            "Please ensure the first sequence in your alignment (after removing gaps and lowercase insertions) "
            "matches the sequence of your PDB structure."
        )
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        # File Inputs
        ttk.Label(self, text="Alignment (fasta/a3m):").grid(row=0, column=0, padx=5, pady=5, sticky='e')
